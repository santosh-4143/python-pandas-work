...
cities = pd.read_csv("lyk_user_city_country.csv",names=["id","userId","country"],index_col=[0],usecols=[0,1,3],header=0)
...
# sorting city wise
intSortCity = cities.sort_values(['userId','country'])
...
for no,user in enumerate(allUsersCity):
    ...
    while True:
        ...
        #Here city is merged
        intUserJoined = pd.merge(intCurrentUser,intNextUser,        how='inner',on='country')
    ...
    if no % 10000 == 0 :
        ...
        intUserFinal.to_csv("usercountrymatch" + str(no) + ".csv",sep = ',')
        intUserFinal = None

intUserFinal = intUserFinal.reset_index(drop=True)
intUserFinal.to_csv("usercountrymatchLast.csv",sep = ',')
intUserFinal = None
